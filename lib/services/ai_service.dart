import 'dart:convert';
import 'dart:io';
import 'package:emotion_journal/services/rag_service.dart';
...
class AIService {
  ...
  AIService() {
    _loadApiKey();
    logger.i(
      'AIService created with synchronous initialization. Consider using AIService.create() instead.',
    );
  }
  ...
  Future<String> acknowledgeFeeling({
    ...
    // Get relevant coping strategies from RAG service
    final relevantEntries = await _ragService.getTopRelevantEntries(
      emotion: emotion,
      journalContent: journalContent,
      pictureDescription: pictureDescription,
    );

    // Build RAG context to include in prompt
    String ragContext = '';
    if (relevantEntries.isNotEmpty) {
      ragContext =
          '\n\nRelevant techniques that might help with this emotion:\n';
      for (int i = 0; i < relevantEntries.length; i++) {
        final entry = relevantEntries[i];
        ragContext += '${i + 1}. ${entry['text']}\n';
      }
    }
...
or perspective shifts that could help. You can reference the relevant techniques I provided if applicable.
  ...
  Future<String> _callOpenAI(String prompt, {File? imageFile}) async {
    ...
    try {
          ...
          'messages': [
            {
              'role': 'system',
              'content':
                  'You are a helpful, empathetic assistant specializing in mental health and well-being.',
            },
            {
              'role': 'user',
              'content': [
      ...
      if (response.statusCode == 200) {
        ...
        logger.e(
          'Error from OpenAI API: ${response.statusCode} - ${response.body}',
        );
